...
################################# INSTRUCTIONS #################################
################################################################################
# To run the in-depth simulation, only the following parameters need to be 
...
model = "D-8" # D- refers to the in-depth model and the number corresponds to those given in our paper

# Complexity parameters
number_effects = c(30, 4)                     # The number of main effects and interactions structured as follows c(# main effects, # first-order interactions, # second-order interactions, # third order interactions, ......)
unique_beta_per_complexity = c(30, 4)         # The number of unique regression coefficients per depth level, structured as follows c(# number of unique regression coefficients main effects, # number of unique regression coefficients first-order effects, .....)
...
n_obs = sapply(seq(4, 44, by = 4), function(x) x^2) # n, the sample size

number_outcome_repeats = 5 # m, the number of replications

# Methods that were used in the in-depth study
methods = c("logistic", "lasso", "ridge", "elastic", "knn", "rf", "svm_linear", "svm_poly", "svm_radial", "gbm", "xgboost")


# This will create the path in which results are stored
dir_path = file.path("results_final", "In-depth-analysis", model)
if (!dir.exists(dir_path)) {
  dir.create(dir_path, recursive = TRUE) 
} 

# Create the file name, which informs about all complexity parameters and date and time
file_name = paste(dir_path, "/D_", format(Sys.time(), "%d-%m-%Y_%H-%M-%S"), "_numeffects=", paste(number_effects, collapse = "_"),
...
# Run simulation
average_df = average_experiments(seeds = c(6, 7, 8),                                      # The random seeds allocated to the in-depth analysis
