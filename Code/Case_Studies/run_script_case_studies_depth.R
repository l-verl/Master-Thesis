...
################################# INSTRUCTIONS #################################
################################################################################
# To run the in-depth analysis on the case studies, only two parameters need to be changed.
...
if (case_study == "framingham") {
  if (balanced == "yes") {
    ...
    n_obs = sapply(seq(4, 40, by = 4), function(x) x^2)
  } else {
    n_obs = sapply(seq(4, 44, by = 4), function(x) x^2)
  }
} else {
  n_obs = sapply(seq(4, 44, by = 4), function(x) x^2)
}
...
average_df = average_experiments(seeds = c(14, 15, 16), 
